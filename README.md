...
```bash
uv run ipython kernel install --user --env VIRTUAL_ENV $(pwd)/.venv --name=sound_recorder_calibration
```
This register this project environment as a kernel `sound_recorder_calibration`, kernels are isolated environments that you can use to run your code.
...
```bash
uvx --from . sound_recorder_calibration
```
